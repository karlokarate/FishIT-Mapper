...
/**
 * JavaScript tracking script to be injected into web pages. This script captures user interactions
 * and reports them to the native app via the JavaScript bridge.
 */
object TrackingScript {

    /**
     * Returns the JavaScript code to inject into web pages for tracking. The script will call
     * methods on the "FishITMapper" JavaScript interface.
     */
    fun getScript(): String =
        ...
        (function() {
            ...
            // Track clicks with link-aware capture
            document.addEventListener('click', function(e) {
                try {
                    ...
                    // Find nearest anchor for href extraction
                    let href = '';
                    const anchor = target.closest ? target.closest('a') : null;
                    if (anchor && anchor.href) {
                        // Get resolved href (absolute URL)
                        href = anchor.href;
                    } else if (target.href) {
                        href = target.href;
                    }

                    if (window.FishITMapper && window.FishITMapper.recordClickExtended) {
                        window.FishITMapper.recordClickExtended(selector, text, x, y, tagName, href, pageUrl);
                    } else if (window.FishITMapper && window.FishITMapper.recordClick) {
            ...
            // Track form submits with extended data
            document.addEventListener('submit', function(e) {
                try {
                    ...
                    if (window.FishITMapper && window.FishITMapper.recordFormSubmitExtended) {
                        window.FishITMapper.recordFormSubmitExtended(formAction, formMethod, fieldNames, pageUrl);
                    } else if (window.FishITMapper && window.FishITMapper.recordFormSubmit) {
